...
function DirectionPicker({ form, setForm, groups, hideLabel = false }) {
  const [open, setOpen] = useState(false); // form.direction 可能是字符串或数字，统一转字符串比较
  const selectedGroup = groups.find((g) => String(g.id) === String(form.direction || ''));
  return (
    <>
      {!hideLabel && (
      ...
      <div className='relative'>
        <button type='button' onClick={() => setOpen((o) => !o)} className='w-full h-8 flex items-center justify-between gap-2 px-3 rounded-md border border-input bg-white text-xs shadow-sm hover:border-gray-300 transition-colors'>
          ...
          <svg className={`h-4 w-4 text-gray-400 flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={2}>
        ...
        {open && (
          <>
            ...
            <div className='absolute z-20 left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden max-h-56 overflow-y-auto'>
              <button
                type='button'
                onClick={() => {
                  setForm((p) => ({
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${!form.direction ? 'bg-gray-50 text-gray-800 font-medium' : 'text-gray-500 hover:bg-gray-50'}`}
              >
                <span className='w-2.5 h-2.5 rounded-full border border-dashed border-gray-300 flex-shrink-0' />
                <span>未分类</span>
                {!form.direction && <Check className='h-3.5 w-3.5 ml-auto text-gray-600' />}
              </button>
              ...
              {groups.map((g, __dnd_i) => (
                <button
                  ...
                  onClick={() => {
                    setForm((p) => ({
                      ...p,
                      direction: String(g.id),
                    }));
                    setOpen(false);
                  }}
                  className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${String(form.direction) === String(g.id) ? 'bg-gray-50' : 'hover:bg-gray-50'}`}
                >
                  <span
                    className='w-2.5 h-2.5 rounded-full flex-shrink-0'
                    style={{
                  ...
                  {String(form.direction) === String(g.id) && (
                    <Check
              ...
              {groups.length === 0 && <div className='px-3 py-4 text-xs text-center text-gray-400'>暂无分组，请前往「配置」创建</div>}
            </div>
          </>
        )}
      </div>
...
function TagSelector({ form, setForm, tags, onTagCreated }) {
  ...
  const selectedIds = form.tag_ids || [];
  const toggleTag = (id) => {
    const sid = String(id);
  ...
  const selectedTags = tags.filter((t) => selectedIds.map(String).includes(String(t.id)));
  ...
  return (
    <div className='relative'>
      <button type='button' onClick={() => setOpen((o) => !o)} className='w-full min-h-[32px] flex items-center flex-wrap gap-1.5 px-2.5 py-1 rounded-md border border-input bg-white text-xs shadow-sm hover:border-gray-300 transition-colors'>
        {selectedTags.length > 0 ? (
          selectedTags.map((t) => (
            <span key={t.id} className='inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium' style={tagStyle(t.color)}>
              {t.name}
            </span>
          ))
        ) : (
          <span className='text-gray-400 text-xs'>选择标签…</span>
        )}
        <svg className={`h-4 w-4 text-gray-400 ml-auto flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={2}>
          <path strokeLinecap='round' strokeLinejoin='round' d='M19 9l-7 7-7-7' />
        </svg>
      </button>
      ...
      {open && (
        <>
          ...
          <div className='absolute z-20 left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden' style={{ minWidth: 160 }}>
            {/* 标签列表 */}
            <div className='max-h-44 overflow-y-auto'>
              {tags.length === 0 && !adding && (
                <div className='px-3 py-3 text-xs text-center text-gray-400'>暂无标签</div>
              )}
              {tags.map((t) => {
                const isSelected = selectedIds.map(String).includes(String(t.id));
                return (
                  <button key={t.id} type='button' onClick={() => toggleTag(t.id)} className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${isSelected ? 'bg-gray-50' : 'hover:bg-gray-50'}`}>
                    <span className='w-2.5 h-2.5 rounded-full flex-shrink-0' style={{ backgroundColor: t.color }} />
                    <span className='truncate font-medium' style={{ color: t.color }}>{t.name}</span>
                    {isSelected && <Check className='h-3.5 w-3.5 ml-auto flex-shrink-0' style={{ color: t.color }} />}
                  </button>
                );
              })}
            </div>
              ...
              {adding ? (
                <div className='px-3 py-2.5 space-y-2' onClick={(e) => e.stopPropagation()}>
                  <input
                    ...
                    placeholder='标签名称…'
                    className='w-full h-7 px-2 text-xs border border-gray-200 rounded-lg outline-none focus:border-[#bbea3b] focus:ring-1 focus:ring-[#bbea3b] transition-colors'
                  />
                  <div className='flex flex-wrap gap-1'>
                    {TAG_PRESET_COLORS.map((c) => (
                      <button
                        ...
                        className={`w-5 h-5 rounded-full flex-shrink-0 transition-transform ${newColor === c ? 'scale-125 ring-2 ring-offset-1 ring-gray-400' : 'hover:scale-110'}`}
                        style={{ backgroundColor: c }}
                      />
                    ))}
                  </div>
                  <div className='flex gap-1.5'>
                    <button type='button' onClick={() => setAdding(false)} className='flex-1 h-7 text-xs text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors'>取消</button>
                    <button
                      type='button' onClick={handleCreateTag}
                      disabled={!newName.trim() || saving}
                      className='flex-1 h-7 text-xs font-medium rounded-lg disabled:opacity-40 transition-colors'
                ...
                >
                  <Plus className='h-3.5 w-3.5' /> 新增标签
                </button>
              )}
            </div>
          </div>
        ...
        <div className='grid grid-cols-2 gap-3 mt-2'>
          <div className='flex items-center gap-1.5 min-w-0'>
            ...
            <div className='flex-1 min-w-0'>
              <DirectionPicker form={fakeForm} setForm={fakeSetFormDirection} groups={groups} hideLabel />
            </div>
          </div>
          <div className='flex items-center gap-1.5 min-w-0'>
            <span className='text-xs font-medium text-gray-400 flex items-center gap-1 flex-shrink-0'>
