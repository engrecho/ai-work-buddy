...
>
> **🟢 默认行为（优先，绝大多数情况）**：用户发来社媒分享链接，只说"保存 / 收藏 / 存一下 / 收趣 / 记下来 / 稍后读"等 → **非离线**，仅 `extract-video` 解析元信息后写入【阅读】列表，**绝不下载文件**。这是默认路径，任何时候都优先走非离线。尤其是抖音 / B站 / 小红书等**视频**链接，视频文件体积大，默认绝不自动下载，以免占用服务端硬盘。
> **🟡 离线仅在"明确"时**：仅当用户清晰表达"离线 / 下载 / 存到本地 / 没网也能看 / 离线收藏"等要把文件落到磁盘的意图时，才走 `download-video`（由服务端下载，文件存**服务端**）。每一条都单独判断是否明确要离线，不因上一条离线过就自动延续。**即使离线，也必须同时写入 title/url/summary/cover_url 四个字段，保证阅读列表信息完整。**
...
**离线下载流程**（走服务端，文件存服务端。即使离线，也必须同时写入 title/url/summary/cover_url 四个字段）：
...
3. AI 调 `createReading(...)` 时把 `is_offline=true`、`offline_path=...` 带上；**同时必须补全 `title`/`url`/`cover_url`/`summary`/`tags`**，让离线条目在阅读列表里信息完整——这与非离线模式的要求完全一致，四个字段缺一不可
4. AI 汇报："已离线保存到服务端（可在 Buddy 网页/App 的阅读列表查看），标题：xxx，链接：xxx，摘要：xxx"
