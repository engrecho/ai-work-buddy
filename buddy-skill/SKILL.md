...
1. AI 收到用户提供的任务信息（标题、描述、时间等）
2. AI 先调用 `list-tasks` 检查是否存在相同或相似标题的任务。如有疑似重复，必须向用户列出并询问是否继续创建或合并。
3. AI 调用 `list-task-groups` 获取当前用户的任务分组，根据任务标题和内容智能判断应归入哪个分组（`--group-id`）。如无法匹配，归入"其他"或默认分组。
4. AI 根据任务的内容和关键词，自动生成 1~3 个标签（`--tags`），标签应反映任务属性（如：工作/生活、项目名、领域等）。
5. 如果用户提到了截止日期或时间（如"下周三"、"7月15号"），AI 必须将其转换为 ISO 8601 格式并写入 `--due` 字段。
6. 默认优先级为 `medium`，除非用户明确说了"紧急"、"重要"等关键词。
7. AI 调用 `add-task` 创建任务，创建后向用户汇报结果（包含ID、标题、分组、标签、截止时间）。

  > 示例流程：
  > 用户："帮我记一下，下周五提交Q3预算方案"
  > 1. list-tasks 检查是否有"预算"相关任务 → 无重复
  > 2. list-task-groups 查看分组 → 归类到"日常管理"
  > 3. 自动打标签 → --tags "预算,工作,Q3"
  > 4. 用户提了截止时间 → --due "2026-07-17T23:59:59+08:00"
  > 5. 创建并汇报

### 用户说"记个备忘" / "添加备忘"

1. AI 收到用户的备忘内容
2. AI 先调用 `list-memos` 检查已存在的备忘，查找是否有高度相似的标题或内容。如有疑似重复，必须向用户展示已有备忘，询问是否合并（update）而非新建。
3. 如果用户确认合并，AI 调用 `update-memo` 将新内容追加到已有备忘；如果不重复或用户选择新建，则调用 `add-memo` 创建。
4. AI 向用户汇报结果（新建或合并、备忘ID、标题）。

  > 示例流程：
  > 用户："团队周会改成周一早10点了"
  > 1. list-memos 检查是否有"周会"相关备忘 → 发现一条"周会安排"的备忘
  > 2. 向用户展示：已有备忘「周会安排」，是否合并？
  > 3. 用户确认合并 → update-memo 追加新内容
  > 4. 汇报结果
...
### 健康档案 API（仅前端使用，未通过 X-API-Key 鉴权）

> ⚠️ 健康档案接口走 `/api/health/*` 前缀（**不是** `/api/v1/*`），使用 JWT Cookie 鉴权（前端），**当前未在 buddy-skill CLI 中暴露**。AI 助手暂不支持通过 API Key 直接查询健康档案。如果未来需要 AI 助手访问健康档案，需后端把 health API 接入 `/api/v1/*` 前缀并支持 X-API-Key 鉴权。

数据模型概览（详见 [docs/DATABASE.md](../docs/DATABASE.md) 第 12-14 节）：

| 表 | 用途 |
|----|------|
| `health_profiles` | 健康档案（就诊人：自己/父母/子女等） |
| `health_visits` | 就诊记录（含报销、下次就诊区间、附件） |
| `health_medications` | 用药记录（按 `visit_id` 分组，未关联就诊的为顶层独立用药） |

前端专用 API 路径（仅供前端调用，AI 助手不可用）：

| 路径 | 方法 | 说明 |
|------|------|------|
| `/api/health` | GET | 系统健康检查（无鉴权） |
| `/api/health/upload` | POST | 上传就诊/药物附件图片（multipart/form-data，返回 `/api/health/images/<file>` URL） |
| `/api/health/profiles/with-stats` | GET | 档案列表（含最近就诊、下次就诊倒计时、当前用药数、就诊总次数） |
| `/api/health/profiles/:id/detail` | GET | 档案详情（含 visits 按日期倒序 + medications 按 `visit_id` 分组到 `visits[].medications`） |
| `/api/health_profiles` | GET/POST | 健康档案 CRUD（通用 `/api/:table` 路由） |
| `/api/health_profiles/:id` | GET/PATCH/DELETE | 单档案 CRUD |
| `/api/health_visits` | GET/POST | 就诊记录 CRUD |
| `/api/health_visits/:id` | GET/PATCH/DELETE | 单就诊记录 CRUD |
| `/api/health_medications` | GET/POST | 用药记录 CRUD |
| `/api/health_medications/:id` | GET/PATCH/DELETE | 单用药记录 CRUD |

**业务约定**：
- `health_visits.is_reimbursed` 是布尔字段（`server/db.js` 已配置 `BOOLEAN_COLUMNS`）
- `health_medications.status` 枚举：`active` / `paused` / `completed` / `as_needed`
- `health_visits.attachment_urls` 是 JSON 数组：`[{url, note?}]`
- `health_visits.next_visit_date` / `next_visit_date_end` 支持日期区间
- 删除档案走软删（`deleted_at`），就诊/用药走物理删
- `GET /api/health/profiles/:id/detail` 返回结构：
  ```json
  { "data": { ...profile, "visits": [{ ...visit, "medications": [...] }], "medications": [...] } }
  ```
  - `visits[0]` 是最新就诊，`visits[0].medications` 是最新就诊的用药
  - 顶层 `medications` 是未关联就诊的独立用药

### 密码保险箱 API（仅前端使用，未通过 X-API-Key 鉴权）

> ⚠️ 密码保险箱接口走 `/api/vault/*` 前缀（**不是** `/api/v1/*`），使用 JWT Cookie + vault_token 双重鉴权，**当前未在 buddy-skill CLI 中暴露**。AI 助手暂不支持访问密码保险箱。如需 AI 助手访问，需后端把 vault API 接入 `/api/v1/*` 前缀并支持 X-API-Key 鉴权。

数据模型概览（详见 [docs/DATABASE.md](../docs/DATABASE.md) 第 15 节）：

| 表 | 用途 |
|----|------|
| `vault_items` | 密码保险箱条目（AES-256-CBC 加密存储密码/备注，软删） |

**字段**：`title` / `category`(password/apikey/card/note) / `username` / `phone` / `email` / `login_methods`(JSON 数组：phone/wechat/qq/google) / `cipher_secret`(加密) / `url` / `cipher_notes`(加密) / `is_active` / `tags`
...
| `/api/vault/items` | GET | 列表（返回元数据，不含明文密码；支持 `category` / `keyword` / `is_active` 查询参数；`keyword` 模糊匹配 `title` / `username` / `phone` / `email`） |
| `/api/vault/items` | POST | 创建条目（前端传明文 `secret` / `notes`，后端 AES 加密存储） |
