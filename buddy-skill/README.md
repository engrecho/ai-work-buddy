...
**默认行为 vs 例外：**
- 🟢 **默认**：用户说「离线保存 / 下载 / 存到本地 / 收藏 / 没网也能看」→ 一律 `download-video`，文件存**服务端**，用户从 Buddy 网页/App 查看。
- 🟡 **唯一例外**：仅当用户**明确说"把文件存到我自己的电脑/本地"**时，才可在用户本机下载一份本地副本；这属于用户本地副本，不应写入 AI-Buddy 的 `is_offline`/离线列表。除此之外，所有"离线保存"都必须走服务端。
