...
// 注意：主流程会调用 resolveOutputRoot() 重新计算,这里仅作为兜底
let OUTPUT_ROOT = ensureOutputRoot();
...
输出根目录优先级：
  1) GV_OUTPUT 环境变量
  2) ~/.all-platform-video-extract/config.json 中的 output_root
  3) <本仓库根目录>/downloads/

其他环境变量：
  GV_NODE      Node 二进制路径（默认 which node 的结果）
  GV_HOST      解析服务地址（默认 https://greenvideo.cc/）
